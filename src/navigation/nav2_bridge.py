    ...
    def spin_once(self) -> None:
        if self._ros_available and self._node:
            try:
                ...
                logger.debug(f"nav2 spin_once skipped: {e}")
    ...
    def destroy(self) -> None:
        if self._ros_available and self._node:
            try:
                ...
                logger.debug(f"nav2 node teardown skipped: {e}")
...
# ── Offline Nav2 map export (no ROS2 required) ────────────────────────────

def export_for_nav2(costmap_2d: np.ndarray,
                    resolution: float,
                    origin_x: float = 0.0,
                    origin_z: float = 0.0,
                    output_dir: str = "output",
                    map_name: str = "phantom_map") -> dict:
    """
    Export costmap as Nav2 offline map: map.pgm + map.yaml.

    This is the standard format that ROS2 `map_server` loads at startup:
        ros2 run nav2_map_server map_server --ros-args -p map_file:=output/phantom_map.yaml

    No ROS2 or network connection is needed to generate this — a robot with Nav2
    can load the file directly. This is the most practical Nav2 integration
    ...
    YAML fields match map_server 2.x (nav2 Humble+):
        image, resolution, origin, negate, occupied_thresh, free_thresh

    Args:
        costmap_2d:   (H, W) int8 array using ROS2 values (0=free, 100=occ, -1=unknown)
        resolution:   meters per cell
        origin_x/z:   world-space coordinates of the map lower-left corner
        output_dir:   directory to write files into
        map_name:     base filename (without extension)

    Returns:
        dict with keys 'pgm_path', 'yaml_path', 'width', 'height', 'n_cells'
    """
    import os
    ...
    # Convert ROS2 costmap values → PGM grayscale
    # ROS2 OccupancyGrid: 0=free, 100=occupied, -1=unknown
    pgm = np.full((H, W), 205, dtype=np.uint8)   # default: unknown (grey)
    pgm[costmap_2d == COSTMAP_FREE]     = 254     # free (near-white)
    pgm[costmap_2d == COSTMAP_OCCUPIED] = 0       # occupied (black)
    # Inscribed / inflation cells
    inscribed_mask = (costmap_2d > COSTMAP_FREE) & (costmap_2d < COSTMAP_OCCUPIED)
    pgm[inscribed_mask] = 127                     # mid-grey for inflation

    # PGM is stored top-row-first; ROS2 map is bottom-row-first
    ...
    yaml_content = (
        ...
        # Nav2 origin = [x, y, yaw] where y here is the Z-axis in PHANTOM coords
        f"origin: [{origin_x:.4f}, {origin_z:.4f}, 0.0]\n"
        f"negate: 0\n"
        f"occupied_thresh: 0.65\n"
        f"free_thresh: 0.196\n"
        f"# Generated by PHANTOM-ECHO REVEAL nav2_bridge.export_for_nav2()\n"
        f"# Load with: ros2 run nav2_map_server map_server "
        f"--ros-args -p map_file:={yaml_path}\n"
    )
    with open(yaml_path, "w") as f:
        f.write(yaml_content)

    n_free = int(np.sum(pgm == 254))
    n_occ  = int(np.sum(pgm == 0))
    n_unk  = int(np.sum(pgm == 205))
    logger.info(
        f"Nav2 map exported: {pgm_path} + {yaml_path} "
        f"({W}x{H} cells, free={n_free}, occ={n_occ}, unknown={n_unk})"
    )
    return {
        "pgm_path":  pgm_path,
        "yaml_path": yaml_path,
        ...
        "load_cmd":  f"ros2 run nav2_map_server map_server --ros-args -p map_file:={yaml_path}",
    }
