...
# Requires only: numpy scipy scikit-learn open3d opencv-venv/Scripts/python.exe-headless pytest.
set -e
cd "$(dirname "$0")"
...
venv/Scripts/python.exe -m src.eval.run_real_eval --dataset datasets/redwood_sample --frames 4 \
...
venv/Scripts/python.exe -m src.main --mode demo > /dev/null
...
venv/Scripts/python.exe -m src.main --mode eval > /dev/null
venv/Scripts/python.exe - <<'PY'
